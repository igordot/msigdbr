...
* in an R-friendly "[tidy](https://r4ds.hadley.nz/data-tidy.html)" format with one gene-to-gene-set mapping per row
...
The `msigdbr()` function retrieves a data frame of gene set memberships, with one row per gene per gene set.
...
```{r msigdbr-c2, eval=FALSE}
cgp_gene_sets <- msigdbr(collection = "C2", subcollection = "CGP")
```

Use `msigdbr_collections()` to check the available collections.

If you require more precise filtering, the `msigdbr()` output is a data frame that can be manipulated using generic methods such as `dplyr::filter()`.

### Converting genes to other species

The `species` parameter converts the human gene identifiers to orthologs of various model organisms.
...
Use `msigdbr_species()` to check the available organisms.
...
Recent releases of MSigDB include both human and mouse versions. The `db_species` parameter selects the underlying MSigDB database (human by default).
...
An [in-depth comparison of the human and mouse databases](hs-mm.html) is available as a separate vignette.

### Understanding the output

The `msigdbr()` function returns a data frame with one gene-to-gene-set mapping per row. Generally, the `gene_symbol` (gene symbol) and `gs_name` (gene set name) columns are the most relevant.
...
The msigdbr output can be used with various pathway analysis packages. It just needs to be reshaped to the appropriate format.
